...
namespace SQL2ClassDemo
{
    ...
    {
        public NameSpaceDatabaseCreate(Table SQLObject, CodeDomProvider provider)
        {
        ...
        public NameSpaceDatabaseCreate(View SQLObject, CodeDomProvider provider)
        {
            this.Name = CodeDomGenerator.GetNameSpace(SQLObject.Owner.Owner.ObjectName, SQLObject.Schema.ObjectName);
    ...
    {
        public ClassTableCreate (Table SQLObject, CodeDomProvider provider)
	    {
            this.IsClass = true;
            this.Name = SQLObject.ObjectName;
    ...
    //Class that Contains the Class reflection of the view.
    public class ClassViewCreate : CodeTypeDeclaration
    {
        public ClassViewCreate(View SQLObject, CodeDomProvider provider)
        {
            this.IsClass = true;
            this.Name = SQLObject.ObjectName;
            if (CodeDomGenerator.MapDescription && !string.IsNullOrEmpty(SQLObject.Description))
    ...
    public class FieldSQLSelectString : CodeMemberField
    {
        ...
        public FieldSQLSelectString(View SQLObject, CodeDomProvider provider)
        {
            string selstr = "SELECT ";
            foreach (Column c in SQLObject.Columns.Items)
                selstr += "[" + c.name + "], ";
            ...
            this.Type = new CodeTypeReference(typeof(string));
            this.Name = CodeDomGenerator.GetFieldName("SQL_Select", provider);
            this.Comments.Add(new CodeCommentStatement("<remarks>This _databasename represents the full SELECT string for the table " + SQLObject.name + ".</remarks>", true));
            this.InitExpression = new CodePrimitiveExpression(selstr);
        }
    }
    ...
    {
        public FieldSQLInsertString(Table SQLObject, CodeDomProvider provider)
        {
            string insstr = "INSERT INTO [" + SQLObject.Schema.name + "].[" + SQLObject.name + "] (";
            foreach (Column c in SQLObject.Columns.Items)
                insstr += "[" + c.name + "], ";
            ...
            this.Comments.Add(new CodeCommentStatement("<remarks>This _databasename represents the full INSERT INTO string for the table " + SQLObject.name + ".</remarks>", true));
    ...
    {
        public FieldSQLUpdateString(Table SQLObject, CodeDomProvider provider)
        {
            string updstr = "UPDATE [" + SQLObject.Schema.name + "].[" + SQLObject.name + "] SET ";
            foreach (Column c in SQLObject.Columns.Items)
                updstr += "[" + c.name + "] = @" + c.name + ", ";
            ...
            else if (SQLObject.HasUniqueIndex)
            {
                ...
                updstr += "WHERE ";
                foreach (IndexColumn ic in I.IndexColumns.Items)
                    updstr += "[" + ic.Column.name + "]=@" + ic.Column.name + " AND ";
                updstr = updstr.Substring(0, updstr.Length - 4);
            }
            this.Attributes = MemberAttributes.Assembly | MemberAttributes.Static;
            this.Type = new CodeTypeReference(typeof(string));
            ...
            if (SQLObject.HasPrimaryKey | SQLObject.HasUniqueIndex)
                this.Comments.Add(new CodeCommentStatement("<remarks>This _databasename represents the full UPDATE string for the table " + SQLObject.name + ", with the WHERE clause.</remarks>", true));
            else
                this.Comments.Add(new CodeCommentStatement("<remarks>This _databasename represents the full UPDATE string for the table " + SQLObject.name + ", without any WHERE clause.</remarks>", true));
    ...
    {
        public FieldSQLDeleteString(Table SQLObject, CodeDomProvider provider)
        {
            string delstr = "DELETE FROM [" + SQLObject.Schema.name + "].[" + SQLObject.name + "] ";
            if (SQLObject.HasPrimaryKey)
            {
                delstr += "WHERE ";
                foreach (Column c in SQLObject.Columns.Items)
            ...
            else if (SQLObject.HasUniqueIndex)
            {
                ...
                delstr += "WHERE ";
                foreach (IndexColumn ic in I.IndexColumns.Items)
                    delstr += "[" + ic.Column.name + "]=@" + ic.Column.name + " AND ";
                delstr = delstr.Substring(0, delstr.Length - 4);
            }
            this.Attributes = MemberAttributes.Assembly | MemberAttributes.Static;
            this.Type = new CodeTypeReference(typeof(string));
            ...
            if (SQLObject.HasPrimaryKey | SQLObject.HasUniqueIndex)
                this.Comments.Add(new CodeCommentStatement("<remarks>This _databasename represents the DELETE string for the table " + SQLObject.name + ", with the WHERE clause.</remarks>", true));
            else
                this.Comments.Add(new CodeCommentStatement("<remarks>This _databasename represents the DELETE string for the table " + SQLObject.name + ", with WHERE clause if there is any Primary keys on the table.</remarks>", true));
    ...
    {
        public FieldColumnCreate(Column SQLObject, CodeDomProvider provider)
        {
            this.Attributes = MemberAttributes.Final | CodeDomGenerator.GetModifier(CodeDomGenerator.FieldModifier);
            CodeTypeReference thistype = new CodeTypeReference(SQLObject.system_type.NetType);
            thistype.BaseType = "System.Nullable";
            this.Type = new CodeTypeReference(SQLObject.system_type.NetType);
            this.Name = CodeDomGenerator.GetFieldName(SQLObject,provider);
            if(CodeDomGenerator.AddComments)
                this.Comments.Add(new CodeCommentStatement("<remarks>SQL Type:" + SQLObject.system_type.name + " - " + SQLObject.Description.Trim() + "</remarks>",true));
        }
    }
    ...
    public class FieldColumnRef : CodeFieldReferenceExpression
    {
        public FieldColumnRef(Column SQLObject, CodeDomProvider provider)
        {
            ...
            if (provider is VBCodeProvider)
                this.FieldName = CodeDomGenerator.VBPreFieldname + SQLObject.name.Replace(" ", "_") + CodeDomGenerator.VBPostFieldname;
            else
    ...
    public class SelectMethod : CodeMemberMethod
    {
        ...
        public SelectMethod(View SQLObject, CodeDomProvider provider)
        {
            this.Attributes = MemberAttributes.Public | MemberAttributes.Final;
            this.Name = "Select";
            ...
            Try.TryStatements.Add(new CodeVariableDeclarationStatement("OleDbConnection", "Conn", new CodeObjectCreateExpression("OleDbConnection", new CodeFieldReferenceExpression(null, "ConnectionString"))));
            Try.TryStatements.Add(new CodeVariableDeclarationStatement("SQLiteCommand", "Com", new CodeMethodInvokeExpression(new CodeTypeReferenceExpression("Conn"), "CreateCommand")));
            Try.TryStatements.Add(new CodeAssignStatement(new CodePropertyReferenceExpression(new CodeTypeReferenceExpression("Com"), "CommandText"), new CodeFieldReferenceExpression(new CodeTypeReferenceExpression(SQLObject.ObjectName), CodeDomGenerator.GetFieldName("SQL_Select", provider))));
    ...
    public class GetSqlParameters : CodeMemberMethod
    {
        ...
        public GetSqlParameters(View SQLObject, CodeDomProvider provider)
        {
            this.ReturnType = new CodeTypeReference("SqlParameter", 1);
            this.Name = "GetSqlParameters";
    ...
    public class AddFromRecordSetMethod : CodeMemberMethod
    {
        ...
        public AddFromRecordSetMethod(View SQLObject, CodeDomProvider provider)
        {
            this.Name = "AddFromRecordSet";
            this.Parameters.Add(new CodeParameterDeclarationExpression("SQLiteDataReader", "rs"));
            CodeMethodReferenceExpression IsDBNullMethod = new CodeMethodReferenceExpression(new CodeSnippetExpression("rs"), "IsDBNull");
    ...
    {
        public ClassFieldEnumeration(Table SQLObject, CodeDomProvider provider)
        {
            this.IsEnum = true;
            this.Name = SQLObject.name + "_Fields";
            foreach (Column c in SQLObject.Columns.Items)
                this.Members.Add(new CodeMemberField(typeof(int), c.name));
            foreach (CodeMemberField cmf in this.Members)
                cmf.Attributes = MemberAttributes.Public;
        }
        public ClassFieldEnumeration(View SQLObject, CodeDomProvider provider)
        {
            this.IsEnum = true;
            this.Name = SQLObject.name + "_Fields";
            foreach (Column c in SQLObject.Columns.Items)
    ...
    {
        public ClassCollection(Table SQLObject, CodeDomProvider provider)
        {
            #region ClassCollection Setup

            this.Name = SQLObject.ObjectName + "Collection";
            this.IsClass = true;
            this.TypeAttributes = TypeAttributes.Public | TypeAttributes.Class;
            this.BaseTypes.Add(typeof(CollectionBase));
            if (!string.IsNullOrEmpty(SQLObject.Description))
                this.CustomAttributes.Add(new CodeAttributeDeclaration("Description", new CodeAttributeArgument(new CodePrimitiveExpression(SQLObject.Description))));
            ...
            AddMethod.CustomAttributes.Add(new CodeAttributeDeclaration("Description", new CodeAttributeArgument(new CodePrimitiveExpression("Adds a new " + SQLObject.name + " to the collection."))));
            AddMethod.Name = "Add";
            AddMethod.ReturnType = new CodeTypeReference(typeof(int));
            AddMethod.Attributes = MemberAttributes.Public | MemberAttributes.Final;
            AddMethod.Parameters.Add(new CodeParameterDeclarationExpression(new CodeTypeReference(SQLObject.name), "item"));
            ...
            RemoveMethod.CustomAttributes.Add(new CodeAttributeDeclaration("Description", new CodeAttributeArgument(new CodePrimitiveExpression("Removes a " + SQLObject.name + " from the collection."))));
            ...
            RemoveMethod.Parameters.Add(new CodeParameterDeclarationExpression(new CodeTypeReference(SQLObject.name), "item"));
            ...
            InsertMethod.CustomAttributes.Add(new CodeAttributeDeclaration("Description", new CodeAttributeArgument(new CodePrimitiveExpression("Inserts an " + SQLObject.name + " into the collection at the specified index."))));
            ...
            InsertMethod.Parameters.Add(new CodeParameterDeclarationExpression(new CodeTypeReference(SQLObject.name), "item"));
            ...
            IndexOfMethod.CustomAttributes.Add(new CodeAttributeDeclaration("Description", new CodeAttributeArgument(new CodePrimitiveExpression("Returns the index value of the " + SQLObject.name + " class in the collection."))));
            ...
            IndexOfMethod.Parameters.Add(new CodeParameterDeclarationExpression(new CodeTypeReference(SQLObject.name), "item"));
            ...
            ContainsMethod.CustomAttributes.Add(new CodeAttributeDeclaration("Description", new CodeAttributeArgument(new CodePrimitiveExpression("Returns true if the " + SQLObject.name + " class is present in the collection."))));
            ...
            ContainsMethod.Parameters.Add(new CodeParameterDeclarationExpression(new CodeTypeReference(SQLObject.name), "item"));
    ...
    {
        public ClassViewCollection(View SQLObject, CodeDomProvider provider)
        {
            #region ClassViewCollection Setup
