...
{
    class CTokenFunctionEX : CTokenFunction
    {


        public CTokenFunctionEX()
        {
            m_token = Token_Type.Token_FunctionEX;


        }
        public CTokenFunctionEX(string _str)
        {
            m_token = Token_Type.Token_FunctionEX;
        ...
        public override bool CheckArgumentValidity(ref List<TypeReference> _vartypelist, ref TypeReference retval, ref string _retsstring)
        {
            try
            {
                int i, j;
                int k;
                int c = this.m_NoOfFunctionArguments;
                switch (tblfunction.FunctionName.ToLower())
                {
                    case "mux":
                        k = 0;
                        if ((tblfunction.m_tblFormalParameterCollection[k].Type & _vartypelist[0].type) == 0)
                        {
                            return false;
                        }
                        k = 1;
                        for (i = k; i < c; i++)
                        {
                            if ((tblfunction.m_tblFormalParameterCollection[k].Type & _vartypelist[i].type) == 0)
                            {
                                return false;
                            }
                        }
                        for (i = k; i < c; i++)
                        {
                            if (Common.IsSimpleType(_vartypelist[i].type))
                            {
                                break;
                            }
                        }

                        retval = _vartypelist[i];
                        for (j = k; j < c; j++)
                        {
                            if (Common.IsSimpleType(_vartypelist[j].type))
                            {
                                if (_vartypelist[j].type != retval.type)
                                {
                                    return false;
                                }
                            }
                    ...
                    case "bools2dint":
                        ...
                        for (i = k; i < c; i++)
                        {
                            if (tblfunction.m_tblFormalParameterCollection[k].Type != _vartypelist[i].type)
                            {
                                return false;
                            }
                        }
                        retval.type = (int)VarType.DINT;
                        break;
