//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     Runtime Version:4.0.30319.42000
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Data;
using System.Data.SQLite;


namespace DCS.DCSTables
{


    public partial class tblDINT : SQLiteTable
	{
		
		#region Static SQL String Memebers
		/// <remarks>This _databasename represents the full INSERT INTO string for the table tblDINT.</remarks>
		internal static string SQL_Insert = "INSERT INTO [tblDINT] ([VarNameID], [SampleTime], [PointGroup], [UNI], [IRL], [IR" +
			"H], [LL], [HH], [L], [H], [RTT], [Interval]) VALUES(@VarNameID, @SampleTime, @Po" +
			"intGroup, @UNI, @IRL, @IRH, @LL, @HH, @L, @H, @RTT, @Interval) ; select last_ins" +
			"ert_rowid(); ";
		
		/// <remarks>This _databasename represents the full UPDATE string for the table tblDINT, without any WHERE clause.</remarks>
		internal static string SQL_Update = "UPDATE [tblDINT] SET [VarNameID] = @VarNameID, [SampleTime] = @SampleTime, [Point" +
			"Group] = @PointGroup, [UNI] = @UNI, [IRL] = @IRL, [IRH] = @IRH, [LL] = @LL, [HH]" +
			" = @HH, [L] = @L, [H] = @H, [RTT] = @RTT, [Interval] = @Interval ";
		
		/// <remarks>This _databasename represents the full SELECT string for the table tblDINT, without any WHERE clause.</remarks>
		internal static string SQL_Select = "SELECT [VarNameID], [SampleTime], [PointGroup], [UNI], [IRL], [IRH], [LL], [HH], " +
			"[L], [H], [RTT], [Interval] FROM [tblDINT] ";
		
		/// <remarks>This _databasename represents the DELETE string for the table tblDINT, with WHERE clause if there is any Primary keys on the table.</remarks>
		internal static string SQL_Delete = "DELETE FROM [tblDINT] ";
		#endregion
		
		#region Tables Memebers
		/// <remarks>SQL Type:System.Int64</remarks>
		private long _VarNameID = -1;
		
		[DisplayName("Var Name ID")]
		[Category("Column")]
		public long VarNameID
		{
			get
			{
				return _VarNameID;
			}
			set
			{
				_VarNameID = value;
			}
		}
		
		/// <remarks>SQL Type:System.Int32</remarks>
		private int _SampleTime;
		
		[DisplayName("Sample Time")]
		[Category("Column")]
		public int SampleTime
		{
			get
			{
				return _SampleTime;
			}
			set
			{
				_SampleTime = value;
			}
		}
		
		/// <remarks>SQL Type:System.Int32</remarks>
		private int _PointGroup;
		
		[DisplayName("Point Group")]
		[Category("Column")]
		public int PointGroup
		{
			get
			{
				return _PointGroup;
			}
			set
			{
				_PointGroup = value;
			}
		}
		
		/// <remarks>SQL Type:System.String</remarks>
		private string _UNI = "";
		
		[DisplayName("UNI")]
		[Category("Column")]
		public string UNI
		{
			get
			{
				return _UNI;
			}
			set
			{
				_UNI = value;
			}
		}
		
		/// <remarks>SQL Type:System.Int32</remarks>
		private int _IRL;
		
		[DisplayName("IRL")]
		[Category("Column")]
		public int IRL
		{
			get
			{
				return _IRL;
			}
			set
			{
				_IRL = value;
			}
		}
		
		/// <remarks>SQL Type:System.Int32</remarks>
		private int _IRH;
		
		[DisplayName("IRH")]
		[Category("Column")]
		public int IRH
		{
			get
			{
				return _IRH;
			}
			set
			{
				_IRH = value;
			}
		}
		
		/// <remarks>SQL Type:System.Int32</remarks>
		private int _LL;
		
		[DisplayName("LL")]
		[Category("Column")]
		public int LL
		{
			get
			{
				return _LL;
			}
			set
			{
				_LL = value;
			}
		}
		
		/// <remarks>SQL Type:System.Int32</remarks>
		private int _HH;
		
		[DisplayName("HH")]
		[Category("Column")]
		public int HH
		{
			get
			{
				return _HH;
			}
			set
			{
				_HH = value;
			}
		}
		
		/// <remarks>SQL Type:System.Int32</remarks>
		private int _L;
		
		[DisplayName("L")]
		[Category("Column")]
		public int L
		{
			get
			{
				return _L;
			}
			set
			{
				_L = value;
			}
		}
		
		/// <remarks>SQL Type:System.Int32</remarks>
		private int _H;
		
		[DisplayName("H")]
		[Category("Column")]
		public int H
		{
			get
			{
				return _H;
			}
			set
			{
				_H = value;
			}
		}
		
		/// <remarks>SQL Type:System.Boolean</remarks>
		private bool _RTT;
		
		[DisplayName("RTT")]
		[Category("Column")]
		public bool RTT
		{
			get
			{
				return _RTT;
			}
			set
			{
				_RTT = value;
			}
		}
		
		/// <remarks>SQL Type:System.Int32</remarks>
		private int _Interval;
		
		[DisplayName("Interval")]
		[Category("Column")]
		public int Interval
		{
			get
			{
				return _Interval;
			}
			set
			{
				_Interval = value;
			}
		}
		#endregion
		
		#region Public Methods
		public int Delete()
		{
			try
			{
				if (Common.Conn == null)
				{
					Common.Conn = new SQLiteConnection(Common.ConnectionString);
					Common.Conn.Open();
				}
				this.PreDeleteTriger();
				SQLiteCommand Com = Common.Conn.CreateCommand();
				SQLiteCommand ComSync = Common.Conn.CreateCommand();
				Com.CommandText = tblDINT.SQL_Delete;
				ComSync.CommandText = "PRAGMA foreign_keys=ON";
				Com.Parameters.AddRange(GetSqlParameters());
				ComSync.ExecuteNonQuery();
				int rowseffected = Com.ExecuteNonQuery();
				ComSync.Dispose();
				Com.Dispose();
				this.PostDeleteTriger();
				return 0;
			}
			catch (SQLiteException ex)
			{
				return ex.ErrorCode;
			}
		}
		
		public int Select()
		{
			try
			{
				if (Common.Conn == null)
				{
					Common.Conn = new SQLiteConnection(Common.ConnectionString);
					Common.Conn.Open();
				}
				SQLiteCommand Com = Common.Conn.CreateCommand();
				Com.CommandText = tblDINT.SQL_Select;
				Com.Parameters.AddRange(GetSqlParameters());
				SQLiteDataReader rs = Com.ExecuteReader();
				for (
				; rs.Read(); 
				)
				{
					AddFromRecordSet(rs);
				}
				rs.Close();
				rs.Dispose();
				Com.Dispose();
				return 0;
			}
			catch (SQLiteException ex)
			{
				return ex.ErrorCode;
			}
		}
		
		public int Insert()
		{
			try
			{
				if (Common.Conn == null)
				{
					Common.Conn = new SQLiteConnection(Common.ConnectionString);
					Common.Conn.Open();
				}
				this.PreInsertTriger();
				SQLiteCommand Com = Common.Conn.CreateCommand();
				Com.CommandText = tblDINT.SQL_Insert;
				Com.Parameters.AddRange(GetSqlParameters());
				Com.Dispose();
				this.PostInsertTriger();
				return 0;
			}
			catch (SQLiteException ex)
			{
				return ex.ErrorCode;
			}
		}
		
		public int Update()
		{
			try
			{
				if (Common.Conn == null)
				{
					Common.Conn = new SQLiteConnection(Common.ConnectionString);
					Common.Conn.Open();
				}
				this.PreUpdateTriger();
				SQLiteCommand Com = Common.Conn.CreateCommand();
				Com.CommandText = tblDINT.SQL_Update;
				Com.Parameters.AddRange(GetSqlParameters());
				int rowseffected = Com.ExecuteNonQuery();
				Com.Dispose();
				this.PostUpdateTriger();
				return 0;
			}
			catch (SQLiteException ex)
			{
				return ex.ErrorCode;
			}
		}
		
		public tblDINT()
		{
		}
		#endregion
		
		#region Private Methods
		private SQLiteParameter[] GetSqlParameters()
		{
			List<SQLiteParameter> SqlParmColl = new List<SQLiteParameter>();
			try
			{
				SqlParmColl.Add(CommonDB.AddSqlParm("@VarNameID", VarNameID, DbType.Int64));
				SqlParmColl.Add(CommonDB.AddSqlParm("@SampleTime", SampleTime, DbType.Int32));
				SqlParmColl.Add(CommonDB.AddSqlParm("@PointGroup", PointGroup, DbType.Int32));
				SqlParmColl.Add(CommonDB.AddSqlParm("@UNI", UNI, DbType.String));
				SqlParmColl.Add(CommonDB.AddSqlParm("@IRL", IRL, DbType.Int32));
				SqlParmColl.Add(CommonDB.AddSqlParm("@IRH", IRH, DbType.Int32));
				SqlParmColl.Add(CommonDB.AddSqlParm("@LL", LL, DbType.Int32));
				SqlParmColl.Add(CommonDB.AddSqlParm("@HH", HH, DbType.Int32));
				SqlParmColl.Add(CommonDB.AddSqlParm("@L", L, DbType.Int32));
				SqlParmColl.Add(CommonDB.AddSqlParm("@H", H, DbType.Int32));
				SqlParmColl.Add(CommonDB.AddSqlParm("@RTT", RTT, DbType.Boolean));
				SqlParmColl.Add(CommonDB.AddSqlParm("@Interval", Interval, DbType.Int32));
				return SqlParmColl.ToArray();
			}
			catch (SQLiteException Exc)
			{
				throw Exc;
			}
		}
		
		public void AddFromRecordSet(SQLiteDataReader rs)
		{
			try
			{
				// if value from the recordset, to the VarNameID _databasename is NOT null then set the value.
				if ((rs.IsDBNull(rs.GetOrdinal("VarNameID")) == false))
				{
					VarNameID = ((long)(Convert.ChangeType(rs["VarNameID"], typeof(long))));
				}
				// if value from the recordset, to the SampleTime _databasename is NOT null then set the value.
				if ((rs.IsDBNull(rs.GetOrdinal("SampleTime")) == false))
				{
					SampleTime = ((int)(Convert.ChangeType(rs["SampleTime"], typeof(int))));
				}
				// if value from the recordset, to the PointGroup _databasename is NOT null then set the value.
				if ((rs.IsDBNull(rs.GetOrdinal("PointGroup")) == false))
				{
					PointGroup = ((int)(Convert.ChangeType(rs["PointGroup"], typeof(int))));
				}
				// if value from the recordset, to the UNI _databasename is NOT null then set the value.
				if ((rs.IsDBNull(rs.GetOrdinal("UNI")) == false))
				{
					UNI = ((string)(Convert.ChangeType(rs["UNI"], typeof(string))));
				}
				// if value from the recordset, to the IRL _databasename is NOT null then set the value.
				if ((rs.IsDBNull(rs.GetOrdinal("IRL")) == false))
				{
					IRL = ((int)(Convert.ChangeType(rs["IRL"], typeof(int))));
				}
				// if value from the recordset, to the IRH _databasename is NOT null then set the value.
				if ((rs.IsDBNull(rs.GetOrdinal("IRH")) == false))
				{
					IRH = ((int)(Convert.ChangeType(rs["IRH"], typeof(int))));
				}
				// if value from the recordset, to the LL _databasename is NOT null then set the value.
				if ((rs.IsDBNull(rs.GetOrdinal("LL")) == false))
				{
					LL = ((int)(Convert.ChangeType(rs["LL"], typeof(int))));
				}
				// if value from the recordset, to the HH _databasename is NOT null then set the value.
				if ((rs.IsDBNull(rs.GetOrdinal("HH")) == false))
				{
					HH = ((int)(Convert.ChangeType(rs["HH"], typeof(int))));
				}
				// if value from the recordset, to the L _databasename is NOT null then set the value.
				if ((rs.IsDBNull(rs.GetOrdinal("L")) == false))
				{
					L = ((int)(Convert.ChangeType(rs["L"], typeof(int))));
				}
				// if value from the recordset, to the H _databasename is NOT null then set the value.
				if ((rs.IsDBNull(rs.GetOrdinal("H")) == false))
				{
					H = ((int)(Convert.ChangeType(rs["H"], typeof(int))));
				}
				// if value from the recordset, to the RTT _databasename is NOT null then set the value.
				if ((rs.IsDBNull(rs.GetOrdinal("RTT")) == false))
				{
					RTT = ((bool)(Convert.ChangeType(rs["RTT"], typeof(bool))));
				}
				// if value from the recordset, to the Interval _databasename is NOT null then set the value.
				if ((rs.IsDBNull(rs.GetOrdinal("Interval")) == false))
				{
					Interval = ((int)(Convert.ChangeType(rs["Interval"], typeof(int))));
				}
			}
			catch (SQLiteException sqlExc)
			{
				throw sqlExc;
			}
			catch (Exception Exc)
			{
				throw Exc;
			}
		}
		
		public override void AddFromString(string[] _strs, string arg1, ref string _log)
		{
			try
			{
				this.PreAddFromString(ref _log);
				int i;
				i = this.ColumnExistInHeader("VarNameID");
				if ((i >= 0))
				{
					VarNameID = ((long)(Convert.ChangeType(_strs[i], typeof(long))));
				}
				i = this.ColumnExistInHeader("SampleTime");
				if ((i >= 0))
				{
					SampleTime = ((int)(Convert.ChangeType(_strs[i], typeof(int))));
				}
				i = this.ColumnExistInHeader("PointGroup");
				if ((i >= 0))
				{
					PointGroup = ((int)(Convert.ChangeType(_strs[i], typeof(int))));
				}
				i = this.ColumnExistInHeader("UNI");
				if ((i >= 0))
				{
					UNI = ((string)(Convert.ChangeType(_strs[i], typeof(string))));
				}
				i = this.ColumnExistInHeader("IRL");
				if ((i >= 0))
				{
					IRL = ((int)(Convert.ChangeType(_strs[i], typeof(int))));
				}
				i = this.ColumnExistInHeader("IRH");
				if ((i >= 0))
				{
					IRH = ((int)(Convert.ChangeType(_strs[i], typeof(int))));
				}
				i = this.ColumnExistInHeader("LL");
				if ((i >= 0))
				{
					LL = ((int)(Convert.ChangeType(_strs[i], typeof(int))));
				}
				i = this.ColumnExistInHeader("HH");
				if ((i >= 0))
				{
					HH = ((int)(Convert.ChangeType(_strs[i], typeof(int))));
				}
				i = this.ColumnExistInHeader("L");
				if ((i >= 0))
				{
					L = ((int)(Convert.ChangeType(_strs[i], typeof(int))));
				}
				i = this.ColumnExistInHeader("H");
				if ((i >= 0))
				{
					H = ((int)(Convert.ChangeType(_strs[i], typeof(int))));
				}
				i = this.ColumnExistInHeader("RTT");
				if ((i >= 0))
				{
					RTT = ((bool)(Convert.ChangeType(_strs[i], typeof(bool))));
				}
				i = this.ColumnExistInHeader("Interval");
				if ((i >= 0))
				{
					Interval = ((int)(Convert.ChangeType(_strs[i], typeof(int))));
				}
				this.PostAddFromString(ref _log);
			}
			catch (SQLiteException sqlExc)
			{
				throw sqlExc;
			}
			catch (Exception Exc)
			{
				throw Exc;
			}
		}
		#endregion
	}
	
	public delegate void tblDINTChangedEventHandler(object sender, System.EventArgs e);
	
	public partial class tblDINTCollection : SQLiteTableCollection
	{
		
		/// <remarks>SQL Type:tblDINTChangedEventHandler</remarks>
		public event tblDINTChangedEventHandler tblDINTChanged;
		
		[Description("Constructor")]
		public tblDINTCollection()
		{
		}
		
		[Description("Invoke the Changed event; called whenever list changes")]
		protected virtual void OntblDINTChanged(System.EventArgs e)
		{
			if (tblDINTChanged != null)
			{
				this.tblDINTChanged(this, e);
			}
		}
		
		[Description("Gets a  tblDINT from the collection.")]
		public tblDINT this[int index]
		{
			get
			{
				return ((tblDINT)(List[index]));
			}
			set
			{
				List[index] = value;
				this.OntblDINTChanged(EventArgs.Empty);
			}
		}
		
		[Description("Gets a  tblDINT from the collection.")]
		public tblDINT Get(int index)
		{
			return ((tblDINT)(List[index]));
		}
		
		[Description("Adds a new tblDINT to the collection.")]
		public void Add(tblDINT item)
		{
			List.Add(item);
			this.OntblDINTChanged(EventArgs.Empty);
		}
		
		[Description("Removes a tblDINT from the collection.")]
		public void Remove(tblDINT item)
		{
			List.Remove(item);
			this.OntblDINTChanged(EventArgs.Empty);
		}
		
		[Description("Inserts an tblDINT into the collection at the specified index.")]
		public void Insert(int index, tblDINT item)
		{
			List.Insert(index, item);
			this.OntblDINTChanged(EventArgs.Empty);
		}
		
		[Description("Returns the index value of the tblDINT class in the collection.")]
		public int IndexOf(tblDINT item)
		{
			return List.IndexOf(item);
		}
		
		[Description("Returns true if the tblDINT class is present in the collection.")]
		public bool Contains(tblDINT item)
		{
			return List.Contains(item);
		}
	}
}
