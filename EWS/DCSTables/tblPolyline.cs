...
namespace DCS.DCSTables
{
    ...
    public partial class tblPolyline : SQLiteTable
	{
		
		#region Static SQL String Memebers
		/// <remarks>This _databasename represents the full INSERT INTO string for the table tblPolyline.</remarks>
		internal static string SQL_Insert = @"INSERT INTO [tblPolyline] ([ID], [DisplayID], [oIndex], [Points], [NoPoints], [LineType], [LastRev], [LineStyle], [Lock], [Visible], [Expression], [Action], [Layer], [Argument], [CompiledExp], [validexpression]) VALUES(@ID, @DisplayID, @oIndex, @Points, @NoPoints, @LineType, @LastRev, @LineStyle, @Lock, @Visible, @Expression, @Action, @Layer, @Argument, @CompiledExp, @validexpression) ; select last_insert_rowid(); ";
		
		/// <remarks>This _databasename represents the full UPDATE string for the table tblPolyline, without any WHERE clause.</remarks>
		internal static string SQL_Update = @"UPDATE [tblPolyline] SET [ID] = @ID, [DisplayID] = @DisplayID, [oIndex] = @oIndex, [Points] = @Points, [NoPoints] = @NoPoints, [LineType] = @LineType, [LastRev] = @LastRev, [LineStyle] = @LineStyle, [Lock] = @Lock, [Visible] = @Visible, [Expression] = @Expression, [Action] = @Action, [Layer] = @Layer, [Argument] = @Argument, [CompiledExp] = @CompiledExp, [validexpression] = @validexpression ";
		
		/// <remarks>This _databasename represents the full SELECT string for the table tblPolyline, without any WHERE clause.</remarks>
		internal static string SQL_Select = "SELECT [ID], [DisplayID], [oIndex], [Points], [NoPoints], [LineType], [LastRev], " +
			"[LineStyle], [Lock], [Visible], [Expression], [Action], [Layer], [Argument], [Co" +
			"mpiledExp], [validexpression] FROM [tblPolyline] ";
		
		/// <remarks>This _databasename represents the DELETE string for the table tblPolyline, with WHERE clause if there is any Primary keys on the table.</remarks>
		internal static string SQL_Delete = "DELETE FROM [tblPolyline] ";
		#endregion
		...
		public int Delete()
		{
			try
			{
				...
				Com.CommandText = tblPolyline.SQL_Delete;
		...
		public int Select()
		{
			try
			{
				...
				SQLiteCommand Com = Common.Conn.CreateCommand();
				Com.CommandText = tblPolyline.SQL_Select;
				Com.Parameters.AddRange(GetSqlParameters());
		...
		public int Insert()
		{
			try
			{
				...
				SQLiteCommand Com = Common.Conn.CreateCommand();
				Com.CommandText = tblPolyline.SQL_Insert;
				Com.Parameters.AddRange(GetSqlParameters());
				Com.Dispose();
		...
		public int Update()
		{
			try
			{
				...
				SQLiteCommand Com = Common.Conn.CreateCommand();
				Com.CommandText = tblPolyline.SQL_Update;
				Com.Parameters.AddRange(GetSqlParameters());
				int rowseffected = Com.ExecuteNonQuery();
				Com.Dispose();
		...
		public override void AddFromString(string[] _strs, string arg1, ref string _log)
		{
			try
			{
				...
				int i;
				i = this.ColumnExistInHeader("ID");
				if ((i >= 0))
				{
					ID = ((long)(Convert.ChangeType(_strs[i], typeof(long))));
				}
	...
	public delegate void tblPolylineChangedEventHandler(object sender, System.EventArgs e);
	
	public partial class tblPolylineCollection : SQLiteTableCollection
	{
		
		/// <remarks>SQL Type:tblPolylineChangedEventHandler</remarks>
		public event tblPolylineChangedEventHandler tblPolylineChanged;
		...
		public tblPolylineCollection(tblDisplay _parent)
		{
			_DisplayID_tblDisplay = _parent;
		}
		
		[Description("Invoke the Changed event; called whenever list changes")]
		protected virtual void OntblPolylineChanged(System.EventArgs e)
		{
			if (tblPolylineChanged != null)
			{
				this.tblPolylineChanged(this, e);
			}
		}
		
		[Description("Gets a  tblPolyline from the collection.")]
		public tblPolyline this[int index]
		{
			get
			{
				return ((tblPolyline)(List[index]));
			}
			set
			{
				List[index] = value;
				this.OntblPolylineChanged(EventArgs.Empty);
			}
		}
		
		[Description("Gets a  tblPolyline from the collection.")]
		public tblPolyline Get(int index)
		{
			return ((tblPolyline)(List[index]));
		}
		
		[Description("Adds a new tblPolyline to the collection.")]
		public void Add(tblPolyline item)
		{
			List.Add(item);
			this.OntblPolylineChanged(EventArgs.Empty);
		}
		
		[Description("Removes a tblPolyline from the collection.")]
		public void Remove(tblPolyline item)
		{
			List.Remove(item);
			this.OntblPolylineChanged(EventArgs.Empty);
		}
		
		[Description("Inserts an tblPolyline into the collection at the specified index.")]
		public void Insert(int index, tblPolyline item)
		{
			List.Insert(index, item);
			this.OntblPolylineChanged(EventArgs.Empty);
		}
		
		[Description("Returns the index value of the tblPolyline class in the collection.")]
		public int IndexOf(tblPolyline item)
		{
			return List.IndexOf(item);
		}
		
		[Description("Returns true if the tblPolyline class is present in the collection.")]
		public bool Contains(tblPolyline item)
		{
