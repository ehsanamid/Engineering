...
namespace DCS.DCSTables
{
    ...
    public partial class tblEditBox : SQLiteTable
	{
		
		#region Static SQL String Memebers
		/// <remarks>This _databasename represents the full INSERT INTO string for the table tblEditBox.</remarks>
		internal static string SQL_Insert = @"INSERT INTO [tblEditBox] ([DisplayID], [oIndex], [Left], [Right], [Top], [Bottom], [Index], [Access], [Enable], [DBLCONF], [Font], [FontColor], [EnableID], [LastRev], [FillColor], [LineStyle], [Lock], [Visible], [Expression], [Action], [CompiledExp], [validexpression]) VALUES(@DisplayID, @oIndex, @Left, @Right, @Top, @Bottom, @Index, @Access, @Enable, @DBLCONF, @Font, @FontColor, @EnableID, @LastRev, @FillColor, @LineStyle, @Lock, @Visible, @Expression, @Action, @CompiledExp, @validexpression) ; select last_insert_rowid(); ";
		
		/// <remarks>This _databasename represents the full UPDATE string for the table tblEditBox, with the WHERE clause.</remarks>
		internal static string SQL_Update = @"UPDATE [tblEditBox] SET [DisplayID] = @DisplayID, [oIndex] = @oIndex, [Left] = @Left, [Right] = @Right, [Top] = @Top, [Bottom] = @Bottom, [Index] = @Index, [Access] = @Access, [Enable] = @Enable, [DBLCONF] = @DBLCONF, [Font] = @Font, [FontColor] = @FontColor, [EnableID] = @EnableID, [LastRev] = @LastRev, [FillColor] = @FillColor, [LineStyle] = @LineStyle, [Lock] = @Lock, [Visible] = @Visible, [Expression] = @Expression, [Action] = @Action, [CompiledExp] = @CompiledExp, [validexpression] = @validexpression WHERE [ID]=@ID ";
		
		/// <remarks>This _databasename represents the full SELECT string for the table tblEditBox, with the WHERE clause.</remarks>
		internal static string SQL_Select = @"SELECT [DisplayID], [oIndex], [Left], [Right], [Top], [Bottom], [Index], [Access], [Enable], [DBLCONF], [Font], [FontColor], [EnableID], [LastRev], [FillColor], [LineStyle], [Lock], [Visible], [Expression], [Action], [CompiledExp], [validexpression] FROM [tblEditBox] WHERE [ID]=@ID ";
		
		/// <remarks>This _databasename represents the DELETE string for the table tblEditBox, with the WHERE clause.</remarks>
		internal static string SQL_Delete = "DELETE FROM [tblEditBox] WHERE [ID]=@ID ";
		#endregion
		...
		public int Delete()
		{
			try
			{
				...
				Com.CommandText = tblEditBox.SQL_Delete;
		...
		public int Select()
		{
			try
			{
				...
				SQLiteCommand Com = Common.Conn.CreateCommand();
				Com.CommandText = tblEditBox.SQL_Select;
				Com.Parameters.AddRange(GetSqlParameters());
		...
		public int Insert()
		{
			try
			{
				...
				SQLiteCommand Com = Common.Conn.CreateCommand();
				Com.CommandText = tblEditBox.SQL_Insert;
				Com.Parameters.AddRange(GetSqlParameters());
		...
		public int Update()
		{
			try
			{
				...
				SQLiteCommand Com = Common.Conn.CreateCommand();
				Com.CommandText = tblEditBox.SQL_Update;
				Com.Parameters.AddRange(GetSqlParameters());
				int rowseffected = Com.ExecuteNonQuery();
				Com.Dispose();
	...
	public delegate void tblEditBoxChangedEventHandler(object sender, System.EventArgs e);
	
	public partial class tblEditBoxCollection : SQLiteTableCollection
	{
		
		/// <remarks>SQL Type:tblEditBoxChangedEventHandler</remarks>
		public event tblEditBoxChangedEventHandler tblEditBoxChanged;
		...
		public tblEditBoxCollection(tblDisplay _parent)
		{
			_DisplayID_tblDisplay = _parent;
		}
		
		[Description("Invoke the Changed event; called whenever list changes")]
		protected virtual void OntblEditBoxChanged(System.EventArgs e)
		{
			if (tblEditBoxChanged != null)
			{
				this.tblEditBoxChanged(this, e);
			}
		}
		
		[Description("Gets a  tblEditBox from the collection.")]
		public tblEditBox this[int index]
		{
			get
			{
				return ((tblEditBox)(List[index]));
			}
			set
			{
				List[index] = value;
				this.OntblEditBoxChanged(EventArgs.Empty);
			}
		}
		
		[Description("Gets a  tblEditBox from the collection.")]
		public tblEditBox Get(int index)
		{
			return ((tblEditBox)(List[index]));
		}
		
		[Description("Adds a new tblEditBox to the collection.")]
		public void Add(tblEditBox item)
		{
			List.Add(item);
			this.OntblEditBoxChanged(EventArgs.Empty);
		}
		
		[Description("Removes a tblEditBox from the collection.")]
		public void Remove(tblEditBox item)
		{
			List.Remove(item);
			this.OntblEditBoxChanged(EventArgs.Empty);
		}
		
		[Description("Inserts an tblEditBox into the collection at the specified index.")]
		public void Insert(int index, tblEditBox item)
		{
			List.Insert(index, item);
			this.OntblEditBoxChanged(EventArgs.Empty);
		}
		
		[Description("Returns the index value of the tblEditBox class in the collection.")]
		public int IndexOf(tblEditBox item)
		{
			return List.IndexOf(item);
		}
		
		[Description("Returns true if the tblEditBox class is present in the collection.")]
		public bool Contains(tblEditBox item)
		{
