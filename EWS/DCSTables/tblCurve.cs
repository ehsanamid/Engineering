...
namespace DCS.DCSTables
{
    ...
    public partial class tblCurve : SQLiteTable
	{
		
		#region Static SQL String Memebers
		/// <remarks>This _databasename represents the full INSERT INTO string for the table tblCurve.</remarks>
		internal static string SQL_Insert = @"INSERT INTO [tblCurve] ([DisplayID], [oIndex], [Points], [LastRev], [LineStyle], [Lock], [Visible], [Expression], [Action], [Layer], [Argument], [CompiledExp], [validexpression]) VALUES(@DisplayID, @oIndex, @Points, @LastRev, @LineStyle, @Lock, @Visible, @Expression, @Action, @Layer, @Argument, @CompiledExp, @validexpression) ; select last_insert_rowid(); ";
		
		/// <remarks>This _databasename represents the full UPDATE string for the table tblCurve, with the WHERE clause.</remarks>
		internal static string SQL_Update = @"UPDATE [tblCurve] SET [DisplayID] = @DisplayID, [oIndex] = @oIndex, [Points] = @Points, [LastRev] = @LastRev, [LineStyle] = @LineStyle, [Lock] = @Lock, [Visible] = @Visible, [Expression] = @Expression, [Action] = @Action, [Layer] = @Layer, [Argument] = @Argument, [CompiledExp] = @CompiledExp, [validexpression] = @validexpression WHERE [ID]=@ID ";
		
		/// <remarks>This _databasename represents the full SELECT string for the table tblCurve, with the WHERE clause.</remarks>
		internal static string SQL_Select = "SELECT [DisplayID], [oIndex], [Points], [LastRev], [LineStyle], [Lock], [Visible]" +
			", [Expression], [Action], [Layer], [Argument], [CompiledExp], [validexpression] " +
			"FROM [tblCurve] WHERE [ID]=@ID ";
		
		/// <remarks>This _databasename represents the DELETE string for the table tblCurve, with the WHERE clause.</remarks>
		internal static string SQL_Delete = "DELETE FROM [tblCurve] WHERE [ID]=@ID ";
		#endregion
		...
		public int Delete()
		{
			try
			{
				...
				Com.CommandText = tblCurve.SQL_Delete;
		...
		public int Select()
		{
			try
			{
				...
				SQLiteCommand Com = Common.Conn.CreateCommand();
				Com.CommandText = tblCurve.SQL_Select;
				Com.Parameters.AddRange(GetSqlParameters());
		...
		public int Insert()
		{
			try
			{
				...
				SQLiteCommand Com = Common.Conn.CreateCommand();
				Com.CommandText = tblCurve.SQL_Insert;
				Com.Parameters.AddRange(GetSqlParameters());
		...
		public int Update()
		{
			try
			{
				...
				SQLiteCommand Com = Common.Conn.CreateCommand();
				Com.CommandText = tblCurve.SQL_Update;
				Com.Parameters.AddRange(GetSqlParameters());
				int rowseffected = Com.ExecuteNonQuery();
				Com.Dispose();
	...
	public delegate void tblCurveChangedEventHandler(object sender, System.EventArgs e);
	
	public partial class tblCurveCollection : SQLiteTableCollection
	{
		
		/// <remarks>SQL Type:tblCurveChangedEventHandler</remarks>
		public event tblCurveChangedEventHandler tblCurveChanged;
		...
		public tblCurveCollection(tblDisplay _parent)
		{
			_DisplayID_tblDisplay = _parent;
		}
		
		[Description("Invoke the Changed event; called whenever list changes")]
		protected virtual void OntblCurveChanged(System.EventArgs e)
		{
			if (tblCurveChanged != null)
			{
				this.tblCurveChanged(this, e);
			}
		}
		
		[Description("Gets a  tblCurve from the collection.")]
		public tblCurve this[int index]
		{
			get
			{
				return ((tblCurve)(List[index]));
			}
			set
			{
				List[index] = value;
				this.OntblCurveChanged(EventArgs.Empty);
			}
		}
		
		[Description("Gets a  tblCurve from the collection.")]
		public tblCurve Get(int index)
		{
			return ((tblCurve)(List[index]));
		}
		
		[Description("Adds a new tblCurve to the collection.")]
		public void Add(tblCurve item)
		{
			List.Add(item);
			this.OntblCurveChanged(EventArgs.Empty);
		}
		
		[Description("Removes a tblCurve from the collection.")]
		public void Remove(tblCurve item)
		{
			List.Remove(item);
			this.OntblCurveChanged(EventArgs.Empty);
		}
		
		[Description("Inserts an tblCurve into the collection at the specified index.")]
		public void Insert(int index, tblCurve item)
		{
			List.Insert(index, item);
			this.OntblCurveChanged(EventArgs.Empty);
		}
		
		[Description("Returns the index value of the tblCurve class in the collection.")]
		public int IndexOf(tblCurve item)
		{
			return List.IndexOf(item);
		}
		
		[Description("Returns true if the tblCurve class is present in the collection.")]
		public bool Contains(tblCurve item)
		{
