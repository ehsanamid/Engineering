...
namespace DCS.Forms
{
    public partial class PlantStructureObjectTypeForm : Form
    {
        public PlantStructureObjectTypeForm()
        {
            InitializeComponent();
        }
        ...
        private void RowExistInSQLDB(string typename, string description)
        {
            int _type = 1;
        ...
        private void buttonOk_Click(object sender, EventArgs e)
        {
            if (CheckDataIsBlank(dataGridView1))
            {
                if (CheckSameNameinGrid(dataGridView1))
                {
                    ...
                    {
                        RowExistInSQLDB((string)dataGridView1.Rows[i].Cells[0].Value, (string)dataGridView1.Rows[i].Cells[1].Value);
                    }
                    bool found;
                    foreach (tblPlantStructureProperty tblplantstructureproperty in tblSolution.m_tblSolution().m_tblPlantStructurePropertyCollection)
                    {
